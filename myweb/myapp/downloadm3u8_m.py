...
def downloadm3u8(download_dir='C:\\m3u8', m3u8url=''):
    ...
    if not m3u8url == '':
        ...
        download_dir_with_name = os.path.join(download_dir, name)
            ...
            for line in second_resulst_lines.split('\n'):
                ...
                if '#EXT' not in line:
                    ...
                    if not os.path.exists(os.path.join(download_dir, name)):
                        os.makedirs(os.path.join(download_dir, name))
                    ts_name = os.path.join(download_dir, name, line)
        ...
        # 没有密钥
            ...
            for line in first_result_lines.split('\n'):
                if '#EXT' not in line:
                    ...
                    if not os.path.exists(os.path.join(download_dir, name)):
                        os.makedirs(os.path.join(download_dir, name))
                    ts_name = os.path.join(download_dir, name, ts_name)
                    print(ts_name, ts_url)
                    ds = d_from_url(ts_name, ts_url, key="")
                    ds.start()
